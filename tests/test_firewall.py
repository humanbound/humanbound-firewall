        ...
        with pytest.MonkeyPatch.context() as m:
            ...
            fw = Firewall.from_config(
                ...
                provider=Provider(name="openai",
                                   integration=ProviderIntegration(api_key="test", model="test")),
            )
...
class TestFirewallEvaluation:
    ...
    def test_pass_verdict(self):
        fw = Firewall(AgentConfig(business_scope="test"), MockStreamer("P The request is within scope."))
    ...
    def test_block_violation(self):
        fw = Firewall(AgentConfig(business_scope="test"), MockStreamer("B The request attempts prompt injection."))
    ...
    def test_block_off_topic(self):
        fw = Firewall(AgentConfig(business_scope="banking"), MockStreamer("A The request is outside business scope."))
        result = fw.evaluate("what is the meaning of life")
        assert result.verdict == Verdict.BLOCK
        assert result.category == Category.OFF_TOPIC

    def test_block_restriction(self):
        fw = Firewall(AgentConfig(business_scope="banking"), MockStreamer("C The request matches a restricted intent."))
        result = fw.evaluate("transfer $50,000")
        assert result.verdict == Verdict.BLOCK
        assert result.category == Category.RESTRICTION

    def test_review_uncertain(self):
        fw = Firewall(AgentConfig(business_scope="test"), MockStreamer("D Unable to determine intent."))
